...
class TestDrVerifyMultiNode(unittest.TestCase):
    ...
    def test_verify_queries_all_reachable_nodes(self, mock_pve_cmd):
        """cmd_dr_backup_verify must SSH to every reachable PVE node."""
        ...
        def cmd_side_effect(cfg, ip, cmd, timeout=30):
            ...
            if "echo OK" in cmd:
                return ("OK", True)
            # Node 1 has VM 100, node 2 has VM 200
            if ip == "10.0.0.1" and "vzdump" in cmd:
                return ("100|1712600000\n", True)
            elif ip == "10.0.0.2" and "vzdump" in cmd:
        ...
        # Both nodes must have been queried for backups (not just reachability)
        backup_queries = [ip for ip in queried_ips if queried_ips.count(ip) >= 2]
        self.assertIn("10.0.0.1", backup_queries, "Node 1 must be queried for backups")
        self.assertIn("10.0.0.2", backup_queries, "Node 2 must be queried for backups")
