...
def _cmd_scan(cfg: FreqConfig, args) -> int:
    ...
    if target and os.path.isdir(target):
        ...
        fmt.line("  Export VMs from vSphere/ESXi as OVA files.")
        fmt.line("  In vSphere: Right-click VM → Export → OVA")
        fmt.blank()
        fmt.line(f"  {fmt.C.PURPLE_BOLD}Step 2: Scan{fmt.C.RESET}")
        fmt.line("  freq dr migrate-vmware scan /path/to/ova-files/")
        fmt.line("  freq dr migrate-vmware scan /path/to/vm.ova")
        fmt.blank()
        fmt.line(f"  {fmt.C.PURPLE_BOLD}Step 3: Import{fmt.C.RESET}")
        fmt.line("  freq dr migrate-vmware import /path/to/vm.ova --vmid 200 --node pve01")
        fmt.blank()
        fmt.line(f"  {fmt.C.PURPLE_BOLD}Step 4: Verify{fmt.C.RESET}")
        fmt.line("  freq list --node pve01")
        fmt.line("  freq vm power start 200")
        fmt.blank()
...
def _cmd_convert(cfg: FreqConfig, args) -> int:
    ...
    fmt.step_start("Converting to QCOW2")
