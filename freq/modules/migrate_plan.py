...
def _generate_recommendations(nodes: list, vms: list) -> list:
    ...
    underloaded = sorted([n for n in nodes if n["mem_pct"] < avg_mem - 5],
                         key=lambda n: n["mem_pct"])
...
def _cmd_show(cfg: FreqConfig, args) -> int:
    ...
    fmt.table_header(
        ("NODE", 12), ("CPU", 8), ("RAM %", 10), ("RAM USED", 12),
        ("RAM TOTAL", 12), ("BALANCE", 10),
    )

    for node in sorted(nodes, key=lambda n: n["mem_pct"], reverse=True):
        mem_color = (fmt.C.RED if node["mem_pct"] > 85 else
                     fmt.C.YELLOW if node["mem_pct"] > 70 else "")
        diff = node["mem_pct"] - avg_mem
        balance = (f"{fmt.C.RED}+{diff:.1f}%{fmt.C.RESET}" if diff > 5 else
                   f"{fmt.C.GREEN}{diff:+.1f}%{fmt.C.RESET}" if diff < -5 else
                   f"{fmt.C.DIM}{diff:+.1f}%{fmt.C.RESET}")
    ...
    if not recommendations:
        ...
        for i, rec in enumerate(recommendations, 1):
            fmt.line(f"  {fmt.C.CYAN}{i}.{fmt.C.RESET} Move "
                     f"{fmt.C.BOLD}VM {rec['vmid']} ({rec['vm_name']}){fmt.C.RESET} "
                     f"[{rec['vm_ram_mb']}MB RAM]")
            fmt.line(f"     {rec['from_node']} ({rec['from_mem_pct']:.1f}% → "
