...
def cmd_why(cfg: FreqConfig, pack, args) -> int:
    ...
    fmt.line("  {b}Category:{r}   {c}{cat}{r}".format(b=fmt.C.BOLD, r=fmt.C.RESET, c=fmt.C.CYAN, cat=cat_name))
    fmt.line("  {b}Description:{r} {d}".format(b=fmt.C.BOLD, r=fmt.C.RESET, d=description))
    fmt.line("  {b}Tier:{r}       {c}{t}{r}".format(b=fmt.C.BOLD, r=fmt.C.RESET, c=fmt.C.CYAN, t=tier))
    fmt.blank()

    # Status flags
    prod_color = fmt.C.RED if is_prod else fmt.C.GREEN
    prod_label = "YES" if is_prod else "no"
    fmt.line("  {b}Production:{r}  {c}{v}{r}".format(b=fmt.C.BOLD, r=fmt.C.RESET, c=prod_color, v=prod_label))

    prot_color = fmt.C.YELLOW if is_protected else fmt.C.GREEN
    prot_label = "YES" if is_protected else "no"
    fmt.line("  {b}Protected:{r}  {c}{v}{r}".format(b=fmt.C.BOLD, r=fmt.C.RESET, c=prot_color, v=prot_label))
    fmt.blank()

    # Allowed actions
    if allowed:
        fmt.line(
            "  {b}Allowed:{r}    {g}{a}{r}".format(b=fmt.C.BOLD, r=fmt.C.RESET, g=fmt.C.GREEN, a=", ".join(allowed))
        )

    # Blocked actions
    if blocked:
        fmt.line(
            "  {b}Blocked:{r}    {red}{a}{r}".format(b=fmt.C.BOLD, r=fmt.C.RESET, red=fmt.C.RED, a=", ".join(blocked))
        )
    ...
    # Helpful explanation
    if cat_name == "unknown":
        fmt.line("  {d}This VMID is not in any fleet-boundaries category.{r}".format(d=fmt.C.DIM, r=fmt.C.RESET))
        fmt.line("  {d}It gets default 'probe' tier (view only).{r}".format(d=fmt.C.DIM, r=fmt.C.RESET))
        fmt.line(
            "  {d}Add it to conf/fleet-boundaries.toml for more permissions.{r}".format(d=fmt.C.DIM, r=fmt.C.RESET)
        )
    elif blocked:
        fmt.line(
            "  {d}Tier '{t}' doesn't allow: {b}{r}".format(d=fmt.C.DIM, t=tier, b=", ".join(blocked), r=fmt.C.RESET)
        )
        fmt.line("  {d}Change tier in conf/fleet-boundaries.toml to unlock.{r}".format(d=fmt.C.DIM, r=fmt.C.RESET))
