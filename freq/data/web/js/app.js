...
function _stampSection(sectionOrId,at){
  var section=typeof sectionOrId==='string'?document.getElementById(sectionOrId):sectionOrId;
  _markPanelFetched(section,at);
}
function _stampFleetSections(at){
  _stampSection('fleet-sec-stats',at);
  _stampSection('fleet-sec-infra',at);
}
function _payloadHasError(payload){
...
function _verifyAndMarkPanelFetch(response,section){
  ...
  try{
    ...
    Promise.resolve(verified).then(function(payload){
      if(_payloadHasError(payload))return;
      _markPanelFetched(section,Date.now());
    }).catch(function(){});
  }catch(e){}
}
...
function _fetchSectionOnExpand(section){
  if(!section||!section.querySelectorAll||section.dataset.expandFetch==='off')return;
  var triggers=section.querySelectorAll(':scope > .section-header [data-action^="load"], :scope > .section-header [data-action^="fetch"], :scope > .section-body [data-action^="load"], :scope > .section-body [data-action^="fetch"]');
  for(var i=0;i<triggers.length;i++){
    var trigger=triggers[i];
    if(trigger.disabled||trigger.dataset.expandFetch==='off'||trigger.hidden)continue;
    var style=getComputedStyle(trigger);
    if(style.display==='none'||style.visibility==='hidden'||!trigger.getClientRects().length)continue;
    trigger.click();
    return;
  }
}
...
var API={
  ...
  VM_POWER:'/api/vm/power',VM_CREATE:'/api/vm/create',VM_CREATE_OPTIONS:'/api/vm/create/options',
  VM_CREATE_PLAN:'/api/vm/create/plan',VM_CREATE_SUBMIT:'/api/vm/create/submit',VM_CREATE_JOB:'/api/vm/create/job',VM_DESTROY:'/api/vm/destroy',
...
function _showApp(){
  ...
  var p1=_authFetch(API.FLEET_OVERVIEW).then(function(r){return r.json()}).then(function(fo){
    fo=fo||{};fo.summary=fo.summary||{};fo.pve_nodes=_arr(fo.pve_nodes);fo.vms=_arr(fo.vms);
    _fleetCache.fo=fo;_initFleetData(fo);_p(40,'FLEET',(fo.summary.total_vms||fo.vms.length||0)+' VMs, '+fo.pve_nodes.length+' nodes');
    return fo;
...
function _silentHealthRefresh(){
  ...
  _authFetch(API.HEALTH,{silent:true}).then(function(r){
    ...
    if(!psBad&&!structReason)_stampFleetSections(Date.now());
  }).catch(function(){
    _healthInFlight=false;
    _healthFailStreak++;
...
function _silentFleetRefresh(){
  ...
  _authFetch(API.FLEET_OVERVIEW,{silent:true}).then(function(r){
    ...
    if(!fpsBad&&!foReason)_stampFleetSections(Date.now());
  }).catch(function(){
    _fleetInFlight=false;
    _fleetFailStreak++;
...
function loadFleetPage(){
  ...
  if(_fleetCache.fo||_fleetCache.hd){
    _renderFleetData(_fleetCache.fo,_fleetCache.hd,_fleetCache.md,_fleetCache.ct,false);
  } else {
...
function refreshLxcWorkloads(){
  _authFetch(API.CT_LIST,{silent:true}).then(function(r){return r.ok?r.json():null;}).then(function(d){
    if(!d||_payloadHasError(d))return;
    _fleetCache.ct=d;
    if(_currentView==='fleet')_renderFleetData(_fleetCache.fo,_fleetCache.hd,_fleetCache.md,_fleetCache.ct,true);
    var section=document.getElementById('fleet-sec-ct');
...
function _infraRoleCard(ph,healthMap){
  ...
  if(up){
    c+='<div class="role-metric"><span class="rm-val c-dim">PENDING</span><span class="rm-lbl">METRICS</span></div>';
  } else {
...
function _enrichInfraCards(){
  _authFetch(API.INFRA_QUICK,{silent:true}).then(function(r){return r.ok?r.json():null;}).then(function(d){
    d=d||{};d.devices=_arr(d.devices);
    ...
    if(!d.devices.length){
      if(ageEl){ageEl.textContent='UNAVAILABLE';ageEl.style.color='var(--yellow)';}
      document.querySelectorAll('[id^="infra-metrics-"]').forEach(function(el){
        if(el)el.innerHTML='<div class="role-metric"><span class="rm-val c-dim">UNAVAILABLE</span><span class="rm-lbl">METRICS</span></div>';
      });
      return;
    }
    d.devices.forEach(function(dev){
    ...
    console.error('infra quick error:',e);
    document.querySelectorAll('[id^="infra-metrics-"]').forEach(function(el){
      if(el)el.innerHTML='<div class="role-metric"><span class="rm-val c-dim">UNAVAILABLE</span><span class="rm-lbl">METRICS</span></div>';
    });
  });
...
function loadMetricsQuick(){
  ...
  Promise.all([
    _authFetch(API.FLEET_OVERVIEW,{silent:true}).then(function(r){return r.ok?r.json():null;}).catch(function(){return null;}),
    _authFetch(API.HEALTH,{silent:true}).then(function(r){return r.ok?r.json():null;}).catch(function(){return null;}),
    _authFetch(API.MEDIA_STATUS,{silent:true}).then(function(r){return r.ok?r.json():null;}).catch(function(){return null;}),
    _authFetch(API.CT_LIST,{silent:true}).then(function(r){return r.ok?r.json():null;}).catch(function(){return null;})
  ]).then(function(results){
    var fo=_payloadHasError(results[0])?null:results[0];
    var hd=_payloadHasError(results[1])?null:results[1];
    var md=_payloadHasError(results[2])?null:results[2];
    var ctd=_payloadHasError(results[3])?null:results[3];
    if(fo)_fleetCache.fo=fo;
    if(hd)_fleetCache.hd=hd;
    if(md)_fleetCache.md=md;
    if(ctd)_fleetCache.ct=ctd;
    _renderFleetData(_fleetCache.fo,_fleetCache.hd,_fleetCache.md,_fleetCache.ct,!!(fo||hd||md||ctd));
  });
}
...
function _buildPveNodeData(pveNodes,healthMap,vmsByNode,ctsByNode,ctrByVmid,labLabels){
  ...
  pveNodes.forEach(function(pn){
    ...
    nodeCard+='<div class="host-actions pve-node-card-actions"><button class="fleet-btn pve-create-btn" title="Create VM on '+nodeName+'" data-action="openVmCreateForNode" data-node="'+nodeName+'">CREATE VM</button></div>';
    nodeCard+='</div>';
    nodeCard+='</div>';
    nodeData[nodeName]={card:nodeCard,vms:'',rows:'',workloadCount:0};
  });
  return nodeData;
}
function _pveWorkloadRow(kind,name,id,status,cpu,ram,klass,nodeColor,attrs){
  var live=String(status||'').toLowerCase()==='running';
  var statusHtml=live?'<span class="c-green">RUNNING</span>':'<span class="c-red">'+_esc(String(status||'unknown').toUpperCase())+'</span>';
  attrs=attrs||'';
  return '<div class="pve-workload-row" '+attrs+'>'+
    '<span class="pve-workload-name" style="color:'+nodeColor+'">'+_esc(name)+'</span>'+
    '<span>'+_esc(kind)+' '+_esc(id||'')+'</span>'+
    '<span>'+statusHtml+'</span>'+
    '<span>'+_esc(cpu||'-')+'</span>'+
    '<span>'+_esc(ram||'-')+'</span>'+
    '<span>'+_esc(klass||'-')+'</span>'+
  '</div>';
}
function _assembleFleetOutput(infraCards,nodeData,pveNodes){
  var out='';
  nodeData=nodeData||{};pveNodes=_arr(pveNodes);
  if(infraCards){
  ...
  nodeOrder.forEach(function(nodeName){
    ...
    var vmCount=nd.workloadCount||0;
    ...
    pveContent+='</div>';
    if(nd.rows){
      pveContent+='<div class="pve-node-actions">';
      pveContent+='<button class="fleet-btn pve-node-toggle" data-action="togglePveGroup" type="button"><span class="pve-chev">'+(isExpanded?'\u25be':'\u25b8')+'</span><span class="pve-toggle-count">'+workloadLabel+'</span><span class="pve-toggle-label">'+(isExpanded?'Hide workloads':'Show workloads')+'</span></button>';
      pveContent+='<span class="pve-node-hint">Click empty detail space to collapse.</span>';
      pveContent+='</div>';
      pveContent+='<div class="pve-vms" data-action="togglePveGroup" style="display:'+(isExpanded?'block':'none')+'">';
      pveContent+='<div class="pve-workload-bar"><span>'+nodeName+' workloads</span><button class="fleet-btn pve-workload-hide" data-action="togglePveGroup" type="button">Collapse</button></div>';
      pveContent+='<div class="pve-workload-table">'+
        '<div class="pve-workload-head"><span>Name</span><span>Kind</span><span>Status</span><span>CPU</span><span>RAM</span><span>Class</span></div>'+
        nd.rows+
      '</div>';
      pveContent+='</div>';
    }
...
function _enrichFleetNtpUpdates(){
  _authFetch(API.FLEET_NTP,{silent:true}).then(function(r){return r.ok?r.json():null;}).then(function(nd){
    var hosts=_arr(nd&&nd.hosts);
    if(!hosts.length){document.querySelectorAll('[id^="ntp-"] .metric-val').forEach(function(el){el.textContent='UNAVAILABLE';el.classList.add('c-dim-fs11');});return;}
  ...
  }).catch(function(e){console.error('API error:',e);document.querySelectorAll('[id^="ntp-"] .metric-val').forEach(function(el){el.textContent='UNAVAILABLE';});});
  _authFetch(API.FLEET_UPDATES,{silent:true}).then(function(r){return r.ok?r.json():null;}).then(function(ud){
    var hosts=_arr(ud&&ud.hosts);
    if(!hosts.length){document.querySelectorAll('[id^="upd-"] .metric-val').forEach(function(el){el.textContent='UNAVAILABLE';el.classList.add('c-dim-fs11');});return;}
    hosts.forEach(function(x){
  ...
  }).catch(function(e){console.error('API error:',e);document.querySelectorAll('[id^="upd-"] .metric-val').forEach(function(el){el.textContent='UNAVAILABLE';});});
}
function _renderFleetData(fo,hd,md,ctd,stampVerifiedWrite){
  try{
    ...
    if(fo){fo.vms=_arr(fo.vms);fo.physical=_arr(fo.physical);fo.pve_nodes=_arr(fo.pve_nodes);fo.summary=fo.summary||{};_normalizeFleetPhysical(fo);}
    if(hd){hd.hosts=_arr(hd.hosts);}
    ...
    foVms.forEach(function(v){
      ...
      if(!nodeData[nodeName])nodeData[nodeName]={card:'',vms:'',rows:'',workloadCount:0};
      ...
      nodeData[nodeName].rows+=_pveWorkloadRow(
        'VM',
        v.name,
        v.vmid,
        v.status,
        running?cpuPct+'% / '+(v.cpu||0)+'c':(v.cpu||0)+'c',
        running?ramPct+'% / '+ramUsedGb+' of '+ramGb:ramGb,
        (v.category&&v.category!=='unknown'?v.category:'')+(v.tier?' / '+v.tier:''),
        cl,
        'data-action="openVmInfo" data-label="'+_esc(v.name)+'" data-vmid="'+v.vmid+'"'
      );
      nodeData[nodeName].workloadCount++;
    });
    Object.keys(ctsByNode).sort().forEach(function(nodeName){
      ...
      ctsByNode[nodeName].forEach(function(c){
        ...
        var cname=c.name||('ct-'+c.ctid);
        var crun=c.status==='running';
        var cRamPct=crun?Math.round(Number(c.mem_pct)||0):0;
        var cRamGB=c.maxmem>0?(c.maxmem/1073741824).toFixed(1).replace(/\.0$/,'')+'GB':'?';
        var cRamUsed=c.mem>0?(c.mem/1073741824).toFixed(1).replace(/\.0$/,'')+'GB':'0GB';
        var cCpuPct=crun?Math.round((Number(c.cpu)||0)*100):0;
        nodeData[nodeName].rows+=_pveWorkloadRow(
          'LXC',
          cname,
          c.ctid,
          c.status,
          crun?cCpuPct+'% / '+(c.maxcpu||0)+'c':(c.maxcpu||0)+'c',
          crun?cRamPct+'% / '+cRamUsed+' of '+cRamGB:cRamGB,
          c.tags||'container',
          _hostColor(cname,'ct',nodeName),
          'data-action="ctTerminal" data-ctid="'+c.ctid+'" data-label="'+_esc('CT '+c.ctid+' '+cname)+'"'
        );
        nodeData[nodeName].workloadCount++;
      });
    });
    ...
    if(stampVerifiedWrite)_stampFleetSections(Date.now());
...
function _vmCreateSelectOptions(items,valueFn,labelFn,selected){
  ...
  _vmCreateArr(items).forEach(function(item){
    ...
    h+='<option value="'+_esc(val)+'"'+(String(val)===String(selected)?' selected':'')+'>'+_esc(labelFn(item))+'</option>';
  });
  return h;
}
...
function vmtPlanCreate(){
  ...
  _authFetch(API.VM_CREATE_PLAN,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)}).then(function(r){return r.json().then(function(d){return {status:r.status,body:d};});}).then(function(res){
    ...
    if(!d.ok||res.status>=400){
      var errs=(d.errors||[d.error||'Plan failed']).join('<br>');
      if(out)out.innerHTML='<div class="c-red" style="padding:10px 0">'+errs+'</div>';
      toast('VM plan failed','error');
      return;
    }
    var plan=d.plan||d;
    ...
    if(out)out.innerHTML='<div class="vmt-plan-card">'+
      ...
      '</div>'+warnHtml+
    '</div>';
    toast('VM plan ready','success');
  }).catch(function(e){
    if(out)out.innerHTML='<div class="c-red" style="padding:10px 0">Plan request failed: '+_esc(e.message||e)+'</div>';
    toast('VM plan request failed','error');
  });
}
function _vmtPollCreateJob(jobId,out){
  if(_vmCreateJobTimer)clearTimeout(_vmCreateJobTimer);
  if(!jobId)return;
  _authFetch(API.VM_CREATE_JOB+'?job_id='+encodeURIComponent(jobId),{silent:true}).then(function(r){return r.json();}).then(function(d){
    var job=(d&&d.job&&typeof d.job==='object')?d.job:d;
    var state=job.state||job.status||d.status||'running';
    var done=d.done||state==='succeeded'||state==='done'||state==='complete';
    var failed=d.failed||state==='failed'||state==='error'||(d.ok===false&&d.error);
    var lines=Array.isArray(job.lines)?job.lines:(Array.isArray(job.logs)?job.logs:[]);
    var log=lines.length?lines.join('\n'):(job.log_tail||job.logs||job.message||job.error||d.error||'');
    if(out)out.innerHTML='<div class="'+(failed?'c-red':done?'c-green':'c-yellow')+'" style="padding:10px 0">'+_esc(state)+'</div><pre style="white-space:pre-wrap;font-size:11px;max-height:180px;overflow:auto">'+_esc(log)+'</pre>';
    if(!done&&!failed)_vmCreateJobTimer=setTimeout(function(){_vmtPollCreateJob(jobId,out);},2000);
  }).catch(function(){_vmCreateJobTimer=setTimeout(function(){_vmtPollCreateJob(jobId,out);},3000);});
}
function _vmCreateJobIdFromResponse(d){
  if(!d)return '';
  if(d.job&&typeof d.job==='object')return d.job.id||d.job.job_id||'';
  if(d.result&&d.result.job&&typeof d.result.job==='object')return d.result.job.id||d.result.job.job_id||'';
  return d.job_id||d.job||d.id||((d.result||{}).job_id)||((d.result||{}).id)||'';
}
...
function vmtCreate(){
  if(document.getElementById('vmt-c-template')){
    ...
    var payload=_vmCreatePayload();
    if(out)out.innerHTML='<div class="c-yellow" style="padding:10px 0">Submitting VM create job...</div>';
    _authFetch(API.VM_CREATE_SUBMIT,{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(payload)}).then(function(r){return r.json().then(function(d){return {status:r.status,body:d};});}).then(function(res){
      var d=res.body||{};
      if(!d.ok||res.status>=400){
        var errs=(d.errors||[d.error||'Create submit failed']).join('<br>');
        if(out)out.innerHTML='<div class="c-red" style="padding:10px 0">'+errs+'</div>';
        toast('VM create submit failed','error');
        return;
      }
      var jobId=_vmCreateJobIdFromResponse(d);
      toast('VM create job submitted','success');
      if(out)out.innerHTML='<div class="c-green" style="padding:10px 0">Create job submitted'+(jobId?': '+_esc(jobId):'.')+'</div>';
      if(jobId)_vmtPollCreateJob(jobId,out);
    }).catch(function(e){
      if(out)out.innerHTML='<div class="c-red" style="padding:10px 0">Create submit failed: '+_esc(e.message||e)+'</div>';
      toast('VM create submit failed','error');
    });
    return;
  }
...
function chaosRun(){
  ...
  confirmAction('Run chaos experiment <strong>"'+_esc(name)+'"</strong> ('+_esc(type)+') on <strong>'+_esc(target)+'</strong>?<br>This will intentionally disrupt the service.',function(){
    ...
    q+='&target='+encodeURIComponent(target)+'&service='+encodeURIComponent(service)+'&duration='+encodeURIComponent(duration);
