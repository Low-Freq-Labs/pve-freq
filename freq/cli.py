...
def _cmd_gwipe(cfg: FreqConfig, pack, args) -> int:
    ...
    if action == "connect":
        # Just test and save
        if not host or not key:
            fmt.line(f"  {fmt.C.RED}Usage: freq gwipe connect --host <ip> --key <apikey>{fmt.C.RESET}")
            return 1
        try:
            ...
            fmt.line(f"  {fmt.C.GREEN}GWIPE station saved: {host}{fmt.C.RESET}")
        except Exception as e:
            fmt.line(f"  {fmt.C.RED}Failed to save: {e}{fmt.C.RESET}")
        return 0

    if not host or not key:
        fmt.header("FREQ WIPE")
        fmt.blank()
        fmt.line(f"  {fmt.C.RED}No wipe station configured.{fmt.C.RESET}")
        fmt.line(f"  {fmt.C.DIM}Connect: freq gwipe connect --host <ip> --key <apikey>{fmt.C.RESET}")
        fmt.line(f"  {fmt.C.DIM}Or set via vault:{fmt.C.RESET}")
        fmt.line(f"  {fmt.C.DIM}  freq vault set gwipe gwipe_host <ip>{fmt.C.RESET}")
        fmt.line(f"  {fmt.C.DIM}  freq vault set gwipe gwipe_api_key <key>{fmt.C.RESET}")
        fmt.blank()
        return 1

    def _gwipe_get(endpoint):
        url = f"http://{host}:7980/api/v1/{endpoint}"
        ...
        try:
            ...
            try:
                return None, json.loads(e.read().decode()).get("error", f"HTTP {e.code}")
            except Exception:
                return None, f"HTTP {e.code}"
        except Exception as e:
            return None, str(e)

    def _gwipe_post(endpoint, body=None):
        url = f"http://{host}:7980/api/v1/{endpoint}"
    ...
    if action == "status":
        d, err = _gwipe_get("status")
        if err:
            fmt.line(f"  {fmt.C.RED}Error: {err}{fmt.C.RESET}")
            return 1
        fmt.line(f"  {fmt.C.PURPLE}Station:{fmt.C.RESET} {host}:7980  {fmt.C.GREEN}v{d.get('version', '?')}{fmt.C.RESET}")
        fmt.line(f"  {fmt.C.PURPLE}Bays:{fmt.C.RESET}    {d.get('bays_total', 0)} total, {d.get('bays_occupied', 0)} occupied")
        fmt.line(f"  {fmt.C.PURPLE}Wiping:{fmt.C.RESET}  {fmt.C.YELLOW}{d.get('wiping', 0)}{fmt.C.RESET}")
        fmt.line(f"  {fmt.C.PURPLE}Wiped:{fmt.C.RESET}   {fmt.C.GREEN}{d.get('wiped', 0)}{fmt.C.RESET}")
        fmt.line(f"  {fmt.C.PURPLE}Failed:{fmt.C.RESET}  {fmt.C.RED}{d.get('failed', 0)}{fmt.C.RESET}")
        fmt.line(f"  {fmt.C.PURPLE}Session:{fmt.C.RESET} {d.get('session_counter', 0)}  |  Lifetime: {d.get('lifetime_counter', 0)}")
        fmt.blank()

    elif action == "bays":
        d, err = _gwipe_get("bays")
        if err:
            fmt.line(f"  {fmt.C.RED}Error: {err}{fmt.C.RESET}")
            return 1
        bays = d.get("bays", {})
        for dev in sorted(bays.keys()):
            ...
            sc = {"WIPING": fmt.C.YELLOW, "WIPED": fmt.C.GREEN, "TESTED": fmt.C.GREEN,
                  "SMART_FAILED": fmt.C.RED, "TESTING": fmt.C.CYAN, "DETECTED": fmt.C.BLUE}.get(state, fmt.C.DIM)
            fmt.line(f"  /dev/{dev}  {sc}{state:15}{fmt.C.RESET}  {model[:30]:30}  {size:10}  {serial}")
            if state == "WIPING":
                ...
                fmt.line(f"           [{bar}] {pct:.1f}%  {speed}  ETA: {eta}")
        fmt.blank()

    elif action == "history":
        d, err = _gwipe_get("history")
        if err:
            fmt.line(f"  {fmt.C.RED}Error: {err}{fmt.C.RESET}")
            return 1
        for e in d.get("history", []):
            rc = fmt.C.GREEN if e.get("result") == "WIPED" else fmt.C.RED
            fmt.line(f"  {e.get('timestamp', '?'):20}  {e.get('bay', '?'):5}  {e.get('model', '?')[:25]:25}  "
                     f"{e.get('size', '?'):10}  {rc}{e.get('result', '?'):8}{fmt.C.RESET}  {e.get('duration', '?')}")
        if not d.get("history"):
            fmt.line(f"  {fmt.C.DIM}No wipe history{fmt.C.RESET}")
        fmt.blank()

    elif action in ("test", "smart"):
        if target:
            d, err = _gwipe_post(f"bays/{target}/smart")
        else:
            d, err = _gwipe_post("test-all")
        if err:
            fmt.line(f"  {fmt.C.RED}Error: {err}{fmt.C.RESET}")
            return 1
        fmt.line(f"  {fmt.C.GREEN}{d.get('message', 'OK')}{fmt.C.RESET}")
        fmt.blank()

    elif action == "wipe":
        if not target:
            fmt.line(f"  {fmt.C.RED}Usage: freq gwipe wipe <bay>  (e.g. freq gwipe wipe sdb){fmt.C.RESET}")
            return 1
        d, err = _gwipe_post(f"bays/{target}/wipe", {"confirm": "YES"})
        if err:
            fmt.line(f"  {fmt.C.RED}Error: {err}{fmt.C.RESET}")
            return 1
        fmt.line(f"  {fmt.C.YELLOW}{d.get('message', 'OK')}{fmt.C.RESET}")
        fmt.blank()

    elif action == "full-send":
        d, err = _gwipe_post("full-send")
        if err:
            fmt.line(f"  {fmt.C.RED}Error: {err}{fmt.C.RESET}")
            return 1
        fmt.line(f"  {fmt.C.GREEN}{d.get('message', 'OK')}{fmt.C.RESET}")
        ...
        if target:
            d, err = _gwipe_post(f"bays/{target}/pause")
        else:
            d, err = _gwipe_post("pause-all")
        if err:
            fmt.line(f"  {fmt.C.RED}Error: {err}{fmt.C.RESET}")
            return 1
        fmt.line(f"  {fmt.C.YELLOW}{d.get('message', 'OK')}{fmt.C.RESET}")
        ...
        if target:
            d, err = _gwipe_post(f"bays/{target}/resume")
        else:
            d, err = _gwipe_post("resume-all")
        if err:
            fmt.line(f"  {fmt.C.RED}Error: {err}{fmt.C.RESET}")
            return 1
        fmt.line(f"  {fmt.C.GREEN}{d.get('message', 'OK')}{fmt.C.RESET}")
        ...
        fmt.line(f"  {fmt.C.DIM}Actions: status | bays | history | test [bay] | wipe <bay> | full-send | pause [bay] | resume [bay] | connect{fmt.C.RESET}")
        fmt.blank()
...
def _cmd_import(cfg: FreqConfig, pack, args) -> int:
    ...
    from freq.jarvis.provision import provision_agent_vm, CLOUD_IMAGES
    from freq.modules.pve import _find_reachable_node, _pve_cmd

    image = getattr(args, "image", None) or "debian-13"
    name = getattr(args, "name", None) or f"freq-{image}"
    vmid = getattr(args, "vmid", None)

    fmt.header(f"Import: {image}")
    fmt.blank()

    if image not in CLOUD_IMAGES:
        fmt.error(f"Unknown image: {image}")
    ...
    fmt.line(f"  {fmt.C.BOLD}Image:{fmt.C.RESET}  {CLOUD_IMAGES[image]['name']}")
    fmt.line(f"  {fmt.C.BOLD}VMID:{fmt.C.RESET}   {vmid}")
    fmt.line(f"  {fmt.C.BOLD}Name:{fmt.C.RESET}   {name}")
    fmt.blank()
