...
**Author:** Morty
**Created:** 2026-04-01
**Updated:** 2026-04-01 (expanded for The Conquest + The Convergence)
**Status:** DRAFT — Do not execute until all features are complete and Sonny approves.
**Companion Docs:** FEATURE-PLAN.md (what to build), THE-CONVERGENCE-OF-PVE-FREQ.md (how to name it)
...
- [ ] `conf/hosts.conf` — all 14 hosts listed with correct types
...
| # | Test | What to Verify |
|---|---|---|
| 21.1 | Start dashboard | `freq serve` on port 8888 |
| 21.2 | Login works | Authentication with configured credentials |
| 21.3 | VM page | VM list, create/destroy buttons, power controls |
| 21.4 | Fleet page | Fleet health, host list, status indicators |
| 21.5 | Network page | Switch interfaces, topology map, SNMP data |
| 21.6 | Firewall page | Rules, NAT, DHCP, gateways |
| 21.7 | Storage page | TrueNAS datasets, ZFS pools, share list |
| 21.8 | DR page | Backup list, SLA compliance, replication status |
| 21.9 | Observability page | Metrics graphs, log viewer, monitors, alerts |
| 21.10 | Security page | Audit scores, compliance %, vuln results, FIM |
| 21.11 | Ops page | Incidents, changes, on-call schedule |
| 21.12 | Docker page | Stacks, containers, update status |
| 21.13 | Hardware page | iDRAC, SMART, UPS, power costs |
| 21.14 | Event page | Event dashboard (if event active) |
| 21.15 | Every API endpoint | Hit /api/v1/<domain>/<action> for each, verify JSON response |
| 21.16 | SSE streaming | Verify live data updates without page refresh |
| 21.17 | Mobile responsive | Check at 375px width (phone), 768px (tablet) |

**Verify:** Every dashboard page loads without JavaScript errors. Every page shows real data (not placeholders). SSE updates work. API endpoints return valid JSON. No page is missing for a domain that exists in the CLI.
...
| # | Test | Command | Expected |
|---|---|---|---|
| 22.1 | Full test suite | `python3 -m pytest tests/ -v` | ALL tests pass, 0 failures |
| 22.2 | Test count | Check pytest output | Must be >= previous count (never goes down) |
| 22.3 | All domain help | For each of ~25 domains: `freq <domain> --help` | Works, shows subcommands |
| 22.4 | All action help | Spot-check 50 random actions: `freq <domain> <action> --help` | Works, shows flags |
| 22.5 | freq doctor | `freq doctor` | 0 failures |
| 22.6 | freq fleet status | `freq fleet status` | All hosts UP |
| 22.7 | API smoke | `freq serve & curl localhost:8888/api/health` | 200 OK |
| 22.8 | Command count | Count all registered actions | Matches expected total from Convergence doc |
| 22.9 | No orphan modules | Check every module file is registered in cli.py | No dead code |
| 22.10 | No old commands | Verify zero flat top-level commands remain (except utilities) | Convergence complete |
...
From RELEASE-STRATEGY.md security checklist. Every item must pass.
...
| 25.4 | conf/ only has examples | `ls conf/*.toml conf/*.conf 2>/dev/null` — only `.example` files |
| 25.5 | .gitignore covers secrets | Verify: `conf/*.toml`, `conf/*.conf`, `data/`, `.env` are ignored |
| 25.6 | No eval/exec on untrusted input | Manual audit of `eval(`, `exec(`, `pickle.load` usage |
| 25.7 | subprocess uses lists not strings | `grep -rn 'shell=True' freq/` — zero matches (or justified) |
| 25.8 | Vault encryption is strong | Verify AES-256 or equivalent |
| 25.9 | API auth uses secure sessions | Verify session tokens, not basic auth |
| 25.10 | Git history clean | `git log --all --diff-filter=A -- '*.pem' '*.key' '.env'` — no secrets ever committed |
